...
public class DoorRotationLite : MonoBehaviour
{
    ...
    public IEnumerator Move()
    {
        ...
        {
            if (openSound != null && closeSound != null && source != null)
            {
                if (State == 1)
                {
            ...
            while (TimeProgression <= Speed)
            {
                TimeProgression += Time.deltaTime;
                float RotationProgression = Mathf.Clamp01(TimeProgression / Speed);
                float RotationCurveValue = rotationcurve.Evaluate(RotationProgression);

                t.rotation = Quaternion.Lerp(InitialRotation, FinalRotation, RotationCurveValue);
