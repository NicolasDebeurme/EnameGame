...
namespace AwesomeTechnologies.VegetationSystem
{
    public partial class VegetationSystemPro
    {
        ...
        public void ClearCacheInverse(Bounds bounds, bool clearBillboards = false)
        {
            // This should mostly be the code below but skips any that DO overlap with bounds

            Rect clearRect = RectExtension.CreateRectFromBounds(bounds);
            CompleteCellLoading();

            //TODO Use quadtree here
            //int count = LoadedVegetationCellList.Count;
            for (int i = LoadedVegetationCellList.Count - 1; i >= 0; i--)
            {
                VegetationCell vegetationCell = LoadedVegetationCellList[i];
            ...
            if (clearBillboards)
            {
                float billboardSize = VegetationSettings.AdditionalTreeMeshDistance + VegetationSettings.AdditionalBillboardDistance;
                bounds.size = new Vector3((bounds.extents.x + billboardSize) * 2, bounds.size.y, (bounds.extents.z + billboardSize) * 2);
                ClearBillboardCellsCacheInverse(bounds);
            }
            //Debug.Log("ClearCache(Bounds): Removed " + (count - LoadedVegetationCellList.Count) + " cells", this);
        }
    }
}