...
namespace AwesomeTechnologies.VegetationStudio
{
    ...
    public partial class VegetationStudioManager : MonoBehaviour
    {
        ...
        public void Internal_ClearCacheInverse(Bounds bounds, bool clearBillboards = false)
        {
            // This should be the same as above just calling the inverse instead
            for (int i = 0; i <= VegetationSystemList.Count - 1; i++)
            {
                VegetationSystemList[i].ClearCacheInverse(bounds, clearBillboards);
            }
        }

        public static void ClearCacheInverse(Bounds bounds, bool clearBillboards = false)
        {
            // This should be the same as above just calling the inverse instead
            if (!Instance)
                FindInstance();
            if (Instance)
            {
                Instance.Internal_ClearCacheInverse(bounds, clearBillboards);
            }
        }
    }
}
