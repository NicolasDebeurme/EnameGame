...
public class SplineComponentEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        {
            Undo.RecordObject(target, "Snap to terrain");
            spline.stickToTerrain = !spline.stickToTerrain;
            spline.ResetIndex();
        }
        if (spline.stickToTerrain)
        {
            EditorGUI.BeginChangeCheck();

            spline.distanceFromTerrain = EditorGUILayout.Slider("Distance from terrain", spline.distanceFromTerrain, 0f, 100f);
            if (EditorGUI.EndChangeCheck())
            {
                Undo.RecordObject(target, "Change distance from terrain");
                spline.ResetIndex();
            }
        }
        GUILayout.EndHorizontal();
    ...
    void OnSceneGUI()
    {
        ...
        for (int i = 0; i < spline.points.Count; i++)
        {
            ...
            if (spline.stickToTerrain)
            { 
                    if (Physics.Raycast(wp, Vector3.down, out RaycastHit hitpoint))
                    {
                        wp.y -= (wp.y - hitpoint.point.y) - spline.distanceFromTerrain;
                    };
            }
    ...
    static void DrawGizmosHiRes(SplineComponent spline, GizmoType gizmoType)
    {
        ...
        if (spline.GetComponent<RoutePicker>().showPoints)
        {
            Gizmos.color = Color.white;
            DrawPoints(spline);
        }
    ...
    static void DrawPoints(SplineComponent spline)
    {
        int pointAmount = spline.GetComponent<RoutePicker>().pointAmount;
        float step = 1f / pointAmount;
        for (int i = 0; i <= pointAmount; i++)
        {
            Gizmos.DrawWireSphere(spline.GetPoint(i * step), .05f);
        }
    }
