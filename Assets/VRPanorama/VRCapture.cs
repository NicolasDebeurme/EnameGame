    ...
    public class VRCapture : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            else
            {
                ...
                if (mute && captureType != VRCaptureList.StillImage)
                ...
                if (captureType == VRCaptureList.AnimationCapture || captureType == VRCaptureList.VRPanoramaRT)
                {
                    ...
                    if (captureAudio)
                    {
                        ...
                        if ((int)AudioSettings.speakerMode == 1)
                            achannels = 1;
                        else if ((int)AudioSettings.speakerMode == 2)
                            achannels = 2;
                        else if ((int)AudioSettings.speakerMode == 3)
                            achannels = 4;
                        else if ((int)AudioSettings.speakerMode == 4)
                            achannels = 5;
                        else if ((int)AudioSettings.speakerMode == 5)
                            achannels = 6;
                        else if ((int)AudioSettings.speakerMode == 6)
                            achannels = 8;
        ...
        void LateUpdate()
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (Input.GetKeyDown(captureKey) && waitRenderFinish == false)
                        step = 1;
            ...
            else
            {
                ...
                if (encodeToMp4)
                {
                    ...
                    if (System.IO.File.Exists(fullPath + Folder + ".wav"))
                    {
                        ...
                        System.Diagnostics.Process.Start(ffmpegPath + "ffmpeg", " -f image2" + " -framerate " + FPS + " -i \"" + fullPath + _prefix + "%05d" + formatString + " -r " + FPS + " -vcodec libx264 -y -pix_fmt yuv420p -b:v " + Mp4Bitrate + "k \"" + fullPath + Folder + ".mp4\"");
                }
        ...
        public void SaveScreenshot()
        {
            ...
            if (ImageFormatType == VRFormatList.JPG)
            {
                ...
                if (captureType == VRCaptureList.StillImage)
                    timestamp = System.DateTime.Now.Ticks + "Screenshot";
        ...
        public Texture2D GetScreenshot(bool eye)
        {
            ...
            if (panoramaType == VRModeList.EquidistantStereo || panoramaType == VRModeList.EquidistantStereoSBS)
            {
                ...
                if (panoramaType == VRModeList.EquidistantStereo)
                    screenShot.ReadPixels(new Rect(0, 0, resolution, resolutionH / 2), 0, 0);
                if (panoramaType == VRModeList.EquidistantStereoSBS)
                    screenShot.ReadPixels(new Rect(0, 0, resolution / 2, resolutionH), 0, 0);
            ...
            else
            {
                ...
                if (encodeToMp4)
                {
                    ...
                    if (System.IO.File.Exists(fullPath + Folder + ".wav"))
                    {
                        ...
                        System.Diagnostics.Process.Start(ffmpegPath + "ffmpeg", " -f image2" + " -framerate " + FPS + " -i \"" + fullPath + _prefix + "%05d" + formatString + " -r " + FPS + " -vcodec libx264 -y -pix_fmt yuv420p -b:v " + Mp4Bitrate + "k \"" + fullPath + Folder + extension + ".mp4\"");
                }
        ...
        public void PreparePano()
        {
            ...
            if (captureType == VRCaptureList.VRPanoramaRT)
            {
                if (captureAngle == VRCaptureAngle._360)
                    ...
                    renderPanorama = (GameObject)Instantiate(Resources.Load("180UnwrappedRT2"));
            ...
            else
            {
                if (captureAngle == VRCaptureAngle._360)
                    ...
                    renderPanorama = (GameObject)Instantiate(Resources.Load("180Unwrapped"));
            }
